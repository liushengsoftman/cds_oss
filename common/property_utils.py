...
class PropertyRules(object):
    ...
    def __init__(self, policy_enforcer=None):
        ...
        self.policies = []
        self.policy_enforcer = policy_enforcer or cdsoss.api.policy.Enforcer()
    ...
    def _add_policy_rules(self, property_exp, action, rule):
        ...
        [prop_a]
        create = cdsoss_creator
        then the corresponding policy rule would be:
        "prop_a:create": "rule:cdsoss_creator"
        where cdsoss_creator is defined in policy.json. For example:
        "cdsoss:creator": "role:admin or role:cdsoss_create_user"
        """
        rule = "rule:%s" % rule
