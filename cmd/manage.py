...
"""
Cdsops Management Utility
"""

from __future__ import print_function

# FIXME(sirp): When we have cdsoss-admin we can consider merging this into it
# Perhaps for consistency with Nova, we would then rename cdsoss-admin ->
# cdsoss-manage (or the other way around)

import os
import sys

# If ../cdsoss/__init__.py exists, add ../ to Python search path, so that
...
def main():
    ...
    try:
        cfg_files = cfg.find_config_files(project='cdsoss',
                                          prog='cdsoss-api')
        #cfg_files.extend(cfg.find_config_files(project='cdsoss',
        #                                       prog='cdsoss-adapter'))
    ...
    except exception.OssException as e:
        sys.exit("ERROR: %s" % e)
